...
//なんか全体的に処理が遅い。例えば登録のsign up押した後に完了するまでに数秒かかっている
import UIKit
import FirebaseAuth
import Firebase
...
struct AuthService {
    ...
    static func resetPassword(withEmail email: String, completion: @escaping(Error?) -> Void) { //, completion: SendPasswordResetCallback?)これが入っていたよ
